...
"%BUN_EXE%" -e "const fs = require('node:fs'); const path = require('node:path'); const root = process.argv[1]; const exe = process.argv[2]; const testsDir = process.argv[3]; const jsonRoot = process.argv[4]; const curlBin = process.platform === 'win32' ? 'curl.exe' : 'curl'; const socketPath = path.join(testsDir, 'ceretree-http.sock'); const request1Path = path.join(testsDir, 'ceretree-http-request-1.json'); const request2Path = path.join(testsDir, 'ceretree-http-request-2.json'); try { fs.rmSync(socketPath, { force: true }); } catch {} fs.writeFileSync(request1Path, JSON.stringify({ jsonrpc: '2.0', id: 21, method: 'system.describe' })); fs.writeFileSync(request2Path, JSON.stringify({ jsonrpc: '2.0', id: 22, method: 'symbols.overview', params: { language: 'go', roots: [jsonRoot], include: 'src/main.go', max_symbols: 5 } })); const server = Bun.spawn([exe, '--server', 'unix://' + socketPath], { cwd: root, stdout: 'ignore', stderr: 'pipe' }); const curl1 = Bun.spawnSync([curlBin, '--silent', '--show-error', '--retry', '20', '--retry-all-errors', '--retry-delay', '0', '--unix-socket', socketPath, '-H', 'content-type: application/json', '-X', 'POST', '--data-binary', '@' + request1Path, 'http://localhost/rpc']); const curl2 = Bun.spawnSync([curlBin, '--silent', '--show-error', '--retry', '5', '--retry-all-errors', '--retry-delay', '0', '--unix-socket', socketPath, '-H', 'content-type: application/json', '-X', 'POST', '--data-binary', '@' + request2Path, 'http://localhost/rpc']); server.kill(); try { fs.rmSync(socketPath, { force: true }); } catch {} if (curl1.exitCode === 0 && curl2.exitCode === 0) { const parsed1 = JSON.parse(Buffer.from(curl1.stdout).toString('utf8')); const parsed2 = JSON.parse(Buffer.from(curl2.stdout).toString('utf8')); if (parsed1.result.server_mode.active === true && parsed1.result.server_mode.target.indexOf('unix://') === 0 && parsed1.result.server_mode.transports.includes('http+unix-socket') === true && Number.isInteger(parsed1.result.process_id) && Array.isArray(parsed2.result.files) && parsed2.result.files.length >= 1) process.exit(0); } process.exit(1);" "%ROOT%" "%EXE%" "%TESTS_DIR%" "%JSON_ROOT%"
if errorlevel 1 exit /b 1

"%BUN_EXE%" -e "const fs = require('node:fs'); const path = require('node:path'); const root = process.argv[1]; const exe = process.argv[2]; const testsDir = process.argv[3]; const curlBin = process.platform === 'win32' ? 'curl.exe' : 'curl'; const socketPath = path.join(testsDir, 'ceretree-http-race.sock'); const request1Path = path.join(testsDir, 'ceretree-http-race-request-1.json'); const request2Path = path.join(testsDir, 'ceretree-http-race-request-2.json'); const response1Path = path.join(testsDir, 'ceretree-http-race-response-1.json'); try { fs.rmSync(socketPath, { force: true }); } catch {} fs.writeFileSync(request1Path, JSON.stringify({ jsonrpc: '2.0', id: 23, method: 'system.describe' })); fs.writeFileSync(request2Path, JSON.stringify({ jsonrpc: '2.0', id: 24, method: 'index.status' })); const curl1 = Bun.spawn([curlBin, '--silent', '--show-error', '--retry', '20', '--retry-all-errors', '--retry-delay', '1', '--unix-socket', socketPath, '-H', 'content-type: application/json', '-X', 'POST', '--data-binary', '@' + request1Path, 'http://localhost/rpc'], { cwd: root, stdout: response1Path, stderr: 'pipe' }); Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, 1200); const server = Bun.spawn([exe, '--server', 'unix://' + socketPath], { cwd: root, stdout: 'ignore', stderr: 'pipe' }); const curl1Exit = curl1.exited; const curl2 = Bun.spawnSync([curlBin, '--silent', '--show-error', '--retry', '5', '--retry-all-errors', '--retry-delay', '0', '--unix-socket', socketPath, '-H', 'content-type: application/json', '-X', 'POST', '--data-binary', '@' + request2Path, 'http://localhost/rpc']); server.kill(); try { fs.rmSync(socketPath, { force: true }); } catch {} if (curl2.exitCode !== 0 || curl1Exit !== 0) process.exit(1); const parsed1 = JSON.parse(fs.readFileSync(response1Path, 'utf8')); const parsed2 = JSON.parse(Buffer.from(curl2.stdout).toString('utf8')); if (parsed1.id === 23 && parsed2.id === 24 && parsed1.result.process_id === parsed2.result.process_id && String(parsed1.result.server_mode.target).indexOf('unix://') === 0) process.exit(0); process.exit(1);" "%ROOT%" "%EXE%" "%TESTS_DIR%"
if errorlevel 1 exit /b 1

"%BUN_EXE%" -e "const fs = require('node:fs'); const path = require('node:path'); const root = process.argv[1]; const exe = process.argv[2]; const testsDir = process.argv[3]; const curlBin = process.platform === 'win32' ? 'curl.exe' : 'curl'; const port = String(28000 + Math.floor(Math.random() * 10000)); const target = 'tcp://127.0.0.1:' + port; const requestPath = path.join(testsDir, 'ceretree-tcp-request.json'); fs.writeFileSync(requestPath, JSON.stringify({ jsonrpc: '2.0', id: 31, method: 'index.status' })); const server = Bun.spawn([exe, '--server', target], { cwd: root, stdout: 'ignore', stderr: 'pipe' }); const curl = Bun.spawnSync([curlBin, '--silent', '--show-error', '--retry', '20', '--retry-all-errors', '--retry-delay', '0', '-H', 'content-type: application/json', '-X', 'POST', '--data-binary', '@' + requestPath, 'http://127.0.0.1:' + port + '/rpc']); server.kill(); if (curl.exitCode === 0) { const parsed = JSON.parse(Buffer.from(curl.stdout).toString('utf8')); if (Array.isArray(parsed.result.roots) && Number.isInteger(parsed.result.process_id)) process.exit(0); } process.exit(1);" "%ROOT%" "%EXE%" "%TESTS_DIR%"
if errorlevel 1 exit /b 1
