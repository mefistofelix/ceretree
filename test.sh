...
"$BUN_BIN" -e "const fs = require('node:fs'); const path = require('node:path'); const root = process.argv[1]; const exe = process.argv[2]; const curlBin = process.platform === 'win32' ? 'curl.exe' : 'curl'; const socketPath = path.join(root, 'tests_cache', 'ceretree-http.sock'); try { fs.rmSync(socketPath, { force: true }); } catch {} const request1 = JSON.stringify({ jsonrpc: '2.0', id: 21, method: 'system.describe' }); const request2 = JSON.stringify({ jsonrpc: '2.0', id: 22, method: 'symbols.overview', params: { language: 'go', roots: [root], include: 'src/main.go', max_symbols: 5 } }); const server = Bun.spawn([exe, '--server', 'unix://' + socketPath], { cwd: root, stdout: 'ignore', stderr: 'pipe' }); let response1 = ''; for (let i = 0; i < 50; i += 1) { const curl = Bun.spawnSync([curlBin, '--silent', '--show-error', '--unix-socket', socketPath, '-H', 'content-type: application/json', '-X', 'POST', '--data-binary', request1, 'http://localhost/rpc']); if (curl.exitCode === 0) { response1 = Buffer.from(curl.stdout).toString('utf8'); break; } Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, 100); } if (!response1) { server.kill(); process.exit(1); } const parsed1 = JSON.parse(response1); const curl2 = Bun.spawnSync([curlBin, '--silent', '--show-error', '--unix-socket', socketPath, '-H', 'content-type: application/json', '-X', 'POST', '--data-binary', request2, 'http://localhost/rpc']); server.kill(); try { fs.rmSync(socketPath, { force: true }); } catch {} if (curl2.exitCode !== 0) process.exit(1); const parsed2 = JSON.parse(Buffer.from(curl2.stdout).toString('utf8')); if (parsed1.result.server_mode.active !== true) process.exit(1); if (!String(parsed1.result.server_mode.target).startsWith('unix://')) process.exit(1); if (!parsed1.result.server_mode.transports.includes('http+unix-socket')) process.exit(1); if (!Array.isArray(parsed2.result.files) || parsed2.result.files.length < 1) process.exit(1);" "$ROOT" "$EXE"

"$BUN_BIN" -e "const root = process.argv[1]; const exe = process.argv[2]; const curlBin = process.platform === 'win32' ? 'curl.exe' : 'curl'; const port = String(28000 + Math.floor(Math.random() * 10000)); const target = 'tcp://127.0.0.1:' + port; const server = Bun.spawn([exe, '--server', target], { cwd: root, stdout: 'ignore', stderr: 'pipe' }); const request = JSON.stringify({ jsonrpc: '2.0', id: 31, method: 'index.status' }); let response = ''; for (let i = 0; i < 50; i += 1) { const curl = Bun.spawnSync([curlBin, '--silent', '--show-error', '-H', 'content-type: application/json', '-X', 'POST', '--data-binary', request, 'http://127.0.0.1:' + port + '/rpc']); if (curl.exitCode === 0) { response = Buffer.from(curl.stdout).toString('utf8'); break; } Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, 100); } server.kill(); if (!response) process.exit(1); const parsed = JSON.parse(response); if (!Array.isArray(parsed.result.roots)) process.exit(1);" "$ROOT" "$EXE"
